...
class GenerationConfig:
    ...
    judges_rules_trajectory_prompts_paths: str = (
        "prompt_templates/judges/pedagogical_stage_check.txt"
    )
...
class TrainConfig:
    ...
    kl_save_thresholds: Optional[list] = field(
        default_factory=lambda: [round(0.05 * i, 10) for i in range(1, 100)],
        metadata={
            "help": "A list of KL divergence thresholds for saving model checkpoints. If the KL divergence between the current model and the reference model exceeds any of these thresholds during training, a checkpoint will be saved for analysis. The default values are [0.05, 0.10, 0.15, ..., 5.0]."
        },
    )
