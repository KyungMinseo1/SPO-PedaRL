...
class GenerationConfig:
    ...
    judges_rules_prompts_paths: Dict[str, str] = field(
        default_factory=lambda: {
            "answer_solution": "prompt_templates/judges/answer_solution_integrated.txt",
            "scaffolding_cognitive_load": "prompt_templates/judges/scaffolding_cognitive_load_integrated.txt",
        }
    )
    ...
    # Turn-wise reward config (no-branch)
    pre_reject_weight_w: float = 1.0
    accuracy_share_mode: str = "uniform"  # "uniform" | "discount"
    accuracy_share_discount_gamma: float = 0.9
    reject_cutoff_enabled: bool = True
    turn_judge_context_turns: int = 0  # 0 means full context up to current teacher turn
