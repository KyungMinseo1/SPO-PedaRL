...
Usage:
    python extract_conversation.py --config-name=7b_2GPU_extract
...
from src.classroom_branch import Classroom
...
def main(cfg: ConversationExtractionConfig):
    ...
    for batch_start in range(0, len(train_dataset), n_problems):
        ...
        # ── wandb logging ───────────────────────────────────────────────────
        if logging_cfg.wandb and rows_for_wandb:
            df = pd.DataFrame(rows_for_wandb)
            wandb.log(
                {
                    f"batch_{batch_num}": wandb.Table(dataframe=df.astype(str)),
                    "total_conversations": total_written,
                    "batch": batch_num,
                    "avg_teacher_turns": _smean(
                        [r["num_teacher_turns"] for r in rows_for_wandb]
                    ),
                    "eoc_rate": _smean(
                        [float(r["ended_with_eoc"]) for r in rows_for_wandb]
                    ),
                }
            )
