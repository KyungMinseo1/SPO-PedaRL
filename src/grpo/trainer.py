...
from src.vllm.client import sample_conversations, wait_batch, get_batch_metrics
...
class ClassroomGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(
        ...
        if self.accelerator.is_local_main_process:
            try:
                batch_metrics = get_batch_metrics(server_port=self.server_port)
                self._metrics[mode]["rewards/accuracy"].append(batch_metrics["accuracy"])
                if batch_metrics["judge_ok_rate"] is not None:
                    self._metrics[mode]["judge/ok_rate"].append(batch_metrics["judge_ok_rate"])
            except Exception as e:
                logger.warning(f"Failed to fetch batch metrics from server: {e}")
