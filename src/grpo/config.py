...
class ClassroomGRPOConfig(TrainingArguments):
    ...
    # NOTE: Add max_group_size for each node + branch_size for the tree search in generation.
    # The total number of generations will be approximately num_generations = (( ((max_turns + 1) / 2) // max_group_size) + 1)^(branch_size).
    max_turns: int = field(
        default=15,
        metadata={
            "help": "Maximum number of turns in the conversation. The actual maximum number of turns will be max_turns + 1."
        },
    )
    max_group_size: int = field(
        default=3,
        metadata={"help": "Maximum number of groups in each node of the tree search."},
    )
    branch_size: int = field(
        default=3,
        metadata={"help": "Number of branches in each node of the tree search."},
    )
