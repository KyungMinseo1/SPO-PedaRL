...
class ClassroomGDPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if args is None:
            ...
            args = ClassroomGDPOConfig(f"{model_name}-GDPO")
        ...
        if (
            ...
            raise ValueError(
                "Invalid `torch_dtype` passed to `GDPOConfig`. Expected either 'auto' or a string representing "
        ...
        def data_collator(features):  # No data collation is needed in GDPO
            return features
    ...
    def _generate_and_score_completions(
        ...
        rewards_per_func_filter = rewards_per_func_filter[(rewards_per_func_filter != -199).any(dim=1)]

        all_reward_advantage = []
        ## Calculate the mean and std of each reward group-wise seperately
        ...
        rewards = (rewards_per_func_filter * self.reward_weights.to(device).unsqueeze(0)).sum(dim=1)
        self._metrics[mode]["reward"].append(rewards.mean().item())
        # Calculate grouped std for logging
        ...
        if (
            ...
            table = {
                ...
                "reward": rewards[process_slice].tolist(),
            }
