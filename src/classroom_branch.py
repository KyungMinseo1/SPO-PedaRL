...
class ConversationState(Enum):
    ...
    TEACHER_TURN = 1
    STUDENT_TURN = 2
    JUDGE_TURN = 3 # Turn level judging
    JUDGE_TURN_T = 4 # Trajectory level judging
    GENERATE_SOLUTION = 5
    REWARD_TURN = 6
    END = 7
...
# Trajectory level judge response
class JudgeDecision_T(BaseModel):
    mistake: JudgeDecision
    concept: JudgeDecision
    verification: JudgeDecision
    application: JudgeDecision
    summary: JudgeDecision

class JudgeReasoning_T(BaseModel):
    mistake: str
    concept: str
    verification: str
    application: str
...
class JudgeResponse_T(BaseModel):
    reasoning: JudgeReasoning_T
    decision: JudgeDecision_T
...
class Conversation:
    def __init__(
        ...
        # Restore trajectory level judge decisions
        self.judge_decisions: list[JudgeResponse_T] = []
    ...
    def _get_hidden_conversation_turn(self, turn_idx: int, turn: TurnPair, n_turns: int):
        messages = []
    ...
    def get_conversation(self, rule_name: Optional[str] = None):
        if self.state == ConversationState.TEACHER_TURN:
            ...
            if rule_name is None:
                raise ValueError("You need to give rule prompts to get the turn judge decisions.")
            return [
                (
                    [
                        {
                            ...
                            "content": Template(
                                open(
                                    self.generation_cfg.judges_rules_turn_prompts_paths[rule_name]
                                ).read()
                            ).render(
                                n_turns=min(self.generation_cfg.n_turns_to_sample, turn_idx+1),
                                conversation=self._get_hidden_conversation_turn(turn_idx, turn, self.generation_cfg.n_turns_to_sample)
                            ),
                        }
                    ],
                    turn
        ...
        elif self.state == ConversationState.JUDGE_TURN_T:
            return [
                {
                    "role": "user",
                    "content": Template(
                        open(
                            self.generation_cfg.judges_rules_trajectory_prompts_paths
                        ).read()
    ...
    def add_trajectory_judge_decisions(self, decisions: List[JudgeResponse_T], num_attempts_required: int):
        if self.state != ConversationState.JUDGE_TURN_T:
            raise ValueError("We are not in the judge turn(trajectory) state")
        self.judge_decisions.extend(decisions)
        for decision in decisions:
            if any(
                v == JudgeDecision.REJECT
                for v in dict(decision.decision).values()
            ):
                self.failed_judges = True
                if not self.generation_cfg.ignore_rejected_judge:
                    self.state = ConversationState.END
                    return
        if len(self.judge_decisions) == num_attempts_required:
            self.state = ConversationState.GENERATE_SOLUTION
    ...
    def get_delta_accuracy(self):
        average_accuracy_reward = (
            sum(self.accuracy_rewards) / len(self.accuracy_rewards) if len(self.accuracy_rewards) > 0 else None
        )
        if average_accuracy_reward is not None:
            delta_accuracy_reward = average_accuracy_reward - self.solve_rate
            return delta_accuracy_reward
        return None

    def get_post_accuracy(self):
        average_accuracy_reward = (
            sum(self.accuracy_rewards) / len(self.accuracy_rewards) if len(self.accuracy_rewards) > 0 else None
        )
        if average_accuracy_reward is not None:
    ...
    def get_pedagogical_stage_alignment_reward(self):
        if len(self.accuracy_rewards) == 0:
            return 0.0
        total_checklists = 0
        total_ok_decisions = 0
        for judge_responses_trajectory in self.judge_decisions:
            for decision in dict(judge_responses_trajectory.decision).values():
                total_checklists += 1
                if decision == JudgeDecision.OK:
                    total_ok_decisions += 1
        return total_ok_decisions / total_checklists if total_checklists > 0 else 0.0
            ...
            [
                {
                    ...
                    "Trajectory Judge Decisions": self.judge_decisions,
...
class Classroom:
    ...
    def generate_next_teacher_utterances(
        ...
        prompts = [conv.get_conversation() for conv in conversations for _ in range(branch_size)]
    ...
    def sample_conversations(
        ...
        if active_rewards is None or "pedagogical_stage_alignment" in active_rewards:
            # We now evaluate the judge rules on the trajectory and add the judge decisions to the conversations.
            logger.info(("=" * 10) + " Running pedagogical judges on each Trajectory " + ("=" * 10))
            start_time = time.time()
            ...
            while any(
                [
                    conversation.state == ConversationState.JUDGE_TURN_T
                    for conversation in conversations
                ]
            ):
                ...
                conversations_to_process = [
                    conv
                    for conv in conversations
                    if conv.state == ConversationState.JUDGE_TURN_T
                ]
                ...
                for conv in conversations_to_process:
                    while len(valid_responses[conv]) < num_attempts_required:
                        ...
                        valid_responses[conv].append(
                            JudgeResponse_T(
                                reasoning=JudgeReasoning_T(
                                    mistake="max attempts reached",
                                    concept="max attempts reached",
                                    verification="max attempts reached",
                                    application="max attempts reached",
                                    summary="max attempts reached",
                                ),
                                decision=JudgeDecision_T(
                                    mistake=JudgeDecision.REJECT,
                                    concept=JudgeDecision.REJECT,
                                    verification=JudgeDecision.REJECT,
                                    application=JudgeDecision.REJECT,
                                    summary=JudgeDecision.REJECT,
                                )
                            )
                        )
                    conv.add_trajectory_judge_decisions(valid_responses[conv], num_attempts_required=num_attempts_required)
    ...
    def run_pedagogical_judges_on_each_turn_pair(self, conversations: List[Conversation]):
        ...
        """
        logger.info("=" * 20 + " Running pedagogical judges on each Turn Pair " + "=" * 20)
        rule_names = self.generation_cfg.judges_rules_turn_prompts_paths.keys()
        ...
        for conv in conversations_to_process:
            for rule_name in rule_names:
                turns_for_judging = conv.get_conversation(rule_name)
        ...
        if not all_judge_tasks:
            ...
            for conv in conversations_to_process:
                conv.state = ConversationState.JUDGE_TURN_T
            return
        ...
        logger.info("Pedagogical judges for turns completed")
    ...
    def compute_all_advantages_flat(
        ...
        rollout_reward_names = [
            r for r in ["accuracy", "end_of_conversation", "pedagogical_stage_alignment"]
            if r in reward_list
        ]
        ...
        for reward_name in rollout_reward_names:
            for problem_idx, convs in problem_to_convs.items():
                ...
                for conv in convs:
                    ...
                    elif reward_name == "pedagogical_stage_alignment":
                        r = conv.get_pedagogical_stage_alignment_reward()
                    else:  # think (rollout)
                        ...
                        for turn_pair in turn_pairs:
                            if turn_pair.is_main_turn:
                                if reward_name == "accuracy":
                                    ...
                                    turn_pair.pedagogical_stage_alignment_advantage = adv_val
                                else:
