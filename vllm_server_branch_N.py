...
def _save_used_sample_ids(sample_ids: List[str]) -> None:
    path = config.dataset.save_selected_sample_ids_path
    if not path:
        return
    ...
    merged = existing | set(sample_ids)
    ...
    logger.info(f"[sample_ids] +{len(sample_ids)} used IDs → {path} (total: {len(merged)})")
...
def sample_conversations_branch(request: ConversationBranchRequest):
    ...
    with lock:
        ...
        if request.sample_ids:
            _save_used_sample_ids(request.sample_ids)
    ...
    for conv in conversations:
        ...
        trajectory_judge_results = {}
        for response in conv.judge_decisions:
            reasoning_dict = response.reasoning.model_dump()
            decision_dict = response.decision.model_dump()
            for key in reasoning_dict:
                trajectory_judge_results.setdefault(key, []).append({
                    "reasoning": reasoning_dict[key],
                    "decision": decision_dict[key].name,
                })
                ...
                turn_pairs_data.append(
                    {
                        ...
                        "teacher_system_prompt": turn_pair.teacher_system_prompt,
        ...
        result_conversations.append(
            {
                ...
                "trajectory_judge_results": trajectory_judge_results,
                "student_persona": conv.student_persona,
                "student_name": conv.student_name,
                "turn_pairs": turn_pairs_data,
                "solutions": conv.solutions,
                "prior_accuracy": conv.get_prior_accuracy(),
                "post_accuracy": conv.get_post_accuracy(),
                "delta_accuracy": conv.get_delta_accuracy(),
                ...
                "pedagogical_stage_alignment_reward": conv.get_pedagogical_stage_alignment_reward(),
                "think_reward": (
                ...
                turn_rows.append(
                    {
                        ...
                        "conversation_id": conv_d["conversation_id"],
                        "teacher_turn": turn_pair["teacher_turn"],
                        "is_main_turn": turn_pair["is_main_turn"],
                        "teacher_system_prompt": turn_pair.get("teacher_system_prompt"),
                        "student_message": turn_pair["student_message"],
                        "teacher_message": turn_pair["teacher_message"],
                        "next_student_message": turn_pair["next_student_message"],
                ...
                rows.append(
                    {
                        ...
                        "trajectory_judge_results": conv_d["trajectory_judge_results"],
                        "student_persona": conv_d["student_persona"],
                        "student_name": conv_d["student_name"],
                        "solutions": conv_d["solutions"],
                        "prior_accuracy": conv_d["prior_accuracy"],
                        "post_accuracy": conv_d["post_accuracy"],
                        "delta_accuracy": conv_d["delta_accuracy"],
                        "accuracy_reward": conv_d.get("accuracy_reward"),
                        "eoc_reward": conv_d.get("end_of_conversation_reward"),
                        "pedagogical_stage_alignment_reward": conv_d.get("pedagogical_stage_alignment_reward"),
